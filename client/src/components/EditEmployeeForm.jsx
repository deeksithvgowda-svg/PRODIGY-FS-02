// client/src/components/EditEmployeeForm.jsx

import React, { useState } from 'react';
import api from '../api';

const EditEmployeeForm = ({ employeeData, onUpdateComplete, onCancel }) => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            await api.put(`/employees/${employeeData._id}`, formData);
            setSuccess('Employee record updated successfully!');
            
            setTimeout(onUpdateComplete, 1000); 

        } catch (err) {
            setError(err.response?.data?.msg || 'Failed to update employee record. Check data format.');
        }
    };
    ...
    return (
        <div style={formStyles.contentCard}>
            <h2 style={formStyles.title}>Edit Employee: {employeeData.first_name} {employeeData.last_name}</h2>
